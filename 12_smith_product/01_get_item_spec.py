...
# 「price¥」に続けて英数字始まりの品番らしき文字列が来ている場合のみ派生品番
# とみなす（日本語の説明文だけの通常価格列は対象外）。品番には「Ⅱ」等の
# ローマ数字が含まれることがあるため、終端は空白または括弧の直前までとする
# （[A-Za-z0-9\-/]のような狭い文字クラスだと「FO-60Ⅱ/B」が「FO-60」で
# 途切れてしまう）。
PRICE_VARIANT_RE = re.compile(r"^price¥([A-Za-z0-9][^\s（(]*)")
